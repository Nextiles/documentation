...
- [NextilesSDK](#nextilessdk)
  ...
  - [Bluetooth Connectivity](#bluetooth-connectivity)
    - [Start and stop scanning](#start-and-stop-scanning)
  ...
  - [Getting Raw Data](#getting-raw-data)
    - [Getting access to PTS for a specific Device](#getting-access-to-pts-for-a-specific-device)
    - [Raw data getters](#raw-data-getters)
  - [Getting Analytics](#getting-analytics)
    - [Get Recent Data](#get-recent-data)
    - [Get Peak Data (Reps)](#get-peak-data-reps)
...
![](assets/choosing-background.png)
...
### Get Peak Data (Reps)
```swift
nextileSdk.getSession(user: currentUser, timeStamp: timeStampString) { success, recentSession in 
   if success {                 
      if let sessionPeaks = recentSession?.peaks {
         for peak in sessionPeaks {
            print("Velocity: \(peak.velocity)")
       }
    }
}
```
...
**DESCRIPTION** <br>
Getting peakanalytics for a specific file which you can loop through each peak (rep) field (e.g. velocity, work, etc).

**Note**
Same as Get Recent Data section above
